...
传统随机标签方法在对象分配和释放时重新选择标签。由于 MTE 只有 4 位标签，相邻对象可能随机获得相同标签，攻击者也可能通过反复尝试或侧信道获知标签，因此空间越界检测只能提供概率保证。

StickyTags 按 size class 组织内存区域，将标签和对象槽位置稳定关联，并让同一 size class 中连续对象使用确定性的 16 标签轮转。这样，跨入前后 15 个对象槽的访问会遇到不同标签，从而为这类有界空间错误提供确定性保护；第 16 个对象槽因标签周期回绕而不在保证范围内。

标签持久性用于避免对象每次分配、释放时反复给内存重新打标签，并维持固定的空间布局。论文明确将 temporal errors（例如 use-after-free）排除在范围外，因此“同一地址复用后标签不变”不能解释为 UAF 防护。

本复现分别验证以下机制：

- **标签轮转**：连续对象的前 16 个标签互不相同，第 17 个对象重新使用第 1 个标签。
- **边界检测**：在同一 size class 中，访问后续第 1 至第 15 个对象槽时产生标签不匹配，第 16 个对象槽因标签周期回绕而不产生标签不匹配。
...
其中，堆栈越界对照、标签轮转、边界距离和标签持久性构成 StickyTags 核心验证；MTE 粒度测试验证的是底层硬件语义，大规模重复矩阵用于补充检查和保留实验依据。上游仓库附带的 `test/test.c` 仅用于观察堆栈地址和标签布局，不执行越界访问，不作为保护效果证据。
...
# 5. 运行保护版本与未保护版本完整机制对照
./Experiment/run-protected-baseline-in-vm.sh 5 20 1000
```

主机脚本从 [统一环境配置](Experiment/lab-env.sh) 读取默认值。协作者可以通过环境变量覆盖实验目录、SSH 密钥、虚拟机地址、端口和用户，无需修改脚本源码。仓库不包含体积较大的 QEMU 系统镜像和构建产物；首次运行前还必须准备 [虚拟机资产](docs/vm-assets.md)。完整步骤见 [构建与运行流程](docs/build-and-run.md)。
